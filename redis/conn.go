...
func (c *conn) readReply() (interface{}, error) {
	...
	switch line[0] {
	case '+', ':':
		return lineWithCRLF, nil
	case '-':
		return Error(string(lineWithCRLF)), nil
		//	case ':':
		//		return line, nil
	case '$':
		...
		return append(append(lineWithCRLF, p...), []byte("\r\n")...), nil
	case '*':
		n, err := parseLen(line[1:])
		if n < 0 || err != nil {
			return nil, err
		...
		buffer.WriteString(string(lineWithCRLF))
		for i := range r {
			r[i], err = c.readReply()
			buffer.WriteString(fmt.Sprintf("%s", r[i]))
			if err != nil {
				return nil, err
			}
		}
...
func (c *conn) readReplyDefault() (interface{}, error) {
	line, err := c.readLine()
	if err != nil {
		return nil, err
	}
